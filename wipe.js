...
/*if (!process.argv[2]){
  console.error('usage : node wipe.js <user_id>');
  process.exit();
}*/
...
db.open(function(e, db_client){
  ...
  var wipeUserMongo = function(user_id){
  ...
  wipeUserRedis(conf.killusr._id, function(){
    console.log('redis wiped');
    wipeUserMongo(conf.killusr_id);
  });
});
